...
def get_predictor_for_user(user_id):
    """Return a Predictor instance with the user's custom session if available."""
    ...
    if user_session:
        ...
        # Use the global predictor (with environment variables)
        return predictor
...
# ---------- Slash Commands (all now respect user session) ----------
...
async def on_ready():
    print(f'✅ >_< Predictor is online as {bot.user}')
    # Sync global commands (or specific guild)
    await bot.tree.sync()
    bot.loop.create_task(win_detector())
...
@bot.tree.command(name="login", description="Authenticate with Bloxflip (Roblox cookie or app.rt/app.at)")
@app_commands.describe(
    roblox_cookie="Your Roblox .ROBLOSECURITY cookie (optional)",
    app_rt="Your Bloxflip app.rt cookie (optional)",
    app_at="Your Bloxflip app.at cookie (optional)"
)
async def login(interaction: discord.Interaction, roblox_cookie: str = None, app_rt: str = None, app_at: str = None):
    await interaction.response.defer(ephemeral=True)
    
    if not any([roblox_cookie, app_rt, app_at]):
        await interaction.followup.send("❌ You must provide at least one authentication method (Roblox cookie or app.rt/app.at).", ephemeral=True)
        return
    
    try:
        predictor.set_auth(roblox_cookie=roblox_cookie, app_rt=app_rt, app_at=app_at)
        # Test the new auth
        resp = predictor.session.get("https://bloxflip.com/api/user/profile")
        if resp.status_code == 200:
            data = resp.json()
            username = data.get('username', 'Unknown')
            await interaction.followup.send(f"✅ Successfully logged in as **{username}**! You can now use prediction commands.", ephemeral=True)
        else:
            await interaction.followup.send(f"❌ Authentication failed (HTTP {resp.status_code}). Please check your cookies/token.", ephemeral=True)
    except Exception as e:
...
@bot.command(name='sync')
async def sync_commands(ctx):
    """Manually sync slash commands to this guild."""
    if not ctx.author.guild_permissions.administrator:
