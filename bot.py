...
@bot.command(name='unrig')
async def unrig(ctx):
    """Reset provably fair seed for Mines"""
    try:
        url = "https://bloxflip.com/api/games/mines/reset-seed"
        resp = predictor.session.post(url, json={})
        if resp.status_code == 200:
            await ctx.send("✅ **Seed reset successfully!** The next Mines game will use a new provably fair seed.")
        else:
            await ctx.send(f"❌ Failed to reset seed (HTTP {resp.status_code}). Response: {resp.text[:200]}")
    except Exception as e:
        await ctx.send(f"❌ Error resetting seed: {str(e)}")
...
async def commands_cmd(ctx):
    ...
    embed.add_field(name="!stats", value="Show accuracy & profit", inline=False)
    embed.add_field(name="!train", value="Retrain models", inline=False)
    embed.add_field(name="!unrig", value="Reset provably fair seed (for Mines)", inline=False)
