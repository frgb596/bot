...
class BloxflipPredictor:
    def __init__(self):
        self.session = requests.Session()
        # Set cookies with correct domain and path
        self.session.cookies.set('app.rt', APP_RT, domain='.bloxflip.com', path='/')
        self.session.cookies.set('app.at', APP_AT, domain='.bloxflip.com', path='/')
    ...
    def _get(self, url, params=None):
        resp = self.session.get(url, params=params, timeout=10)
        if resp.status_code == 403:
            raise Exception("403 Forbidden – check your app.rt and app.at cookies. They may be expired or invalid.")
        resp.raise_for_status()
        return resp.json()
...
class BloxflipPredictor:
    def __init__(self):
        self.session = requests.Session()
        # Set cookies
        self.session.cookies.set('app.rt', APP_RT, domain='.bloxflip.com')
        self.session.cookies.set('app.at', APP_AT, domain='.bloxflip.com')
        # If an Authorization header is provided, use it too (some endpoints prefer it)
        if BLOXFLIP_AUTH:
            self.session.headers.update({'Authorization': BLOXFLIP_AUTH})
    ...
    def predict_mines(self):
        grid = self.get_current_mines_grid()
        if grid is None or self.mines_model is None:
            return None, None, None
        X = grid.reshape(1, 5, 5, 1)
        probs = self.mines_model.predict(X, verbose=0)[0]
        prob_grid = probs.reshape(5, 5)
